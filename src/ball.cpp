...
Ball::Ball() : _bbox(new Circle(Vec3(), 10)), _m(1), objType("Object") {}

Ball::Ball(BBox *bounds, double mass, const Vec3 &position,
           const Vec3 &velocity, const double &elasticity, const Vec3 &aVel)
  ...
  std::cerr << "obj \"" << objType << "\" create pos " << _bbox->pos()
            << " vel " << _vel << " elast " << elasticity << "\n";
}

Ball::~Ball() { std::cerr << "obj delete pos " << _bbox->pos() << "\n"; }
...
void Ball::resolveCollision(Ball &otherObj, double dt) {
  ...
  Vec3 torqueArm = positionDiff.unit() * static_cast<Circle *>(_bbox)->radius();
...
void Ball::move(double dt, Vec3 outsideEnv) {
  ...
  if (outsideEnv.mag() > 0.01) {
    ...
    Vec3 torqueArm = outsideEnv.unit() * static_cast<Circle *>(_bbox)->radius();
    Vec3 velContactPoint = _vel + _aVel.cross(torqueArm);
    Vec3 frictionForce =
        -velContactPoint * simParams.objFrictionCoeff * reactiveForce.mag();
  ...
  _aAccel = _tNet / (0.4 * _m * pow(static_cast<Circle *>(_bbox)->radius(), 2));
