...
Object::Object()
    : _bbox(new Circle(Vec2(), 10)), _m(1), objType("Object") {}
...
Object::Object(BBox *bounds, double mass, const Vec2 &position,
    ...
    : _bbox(bounds), _m(mass), _vel(velocity), _aVel(aVel),
      _k(simParams.objSpringCoeff),
      _b(simParams.objSpringDamping),
...
void Object::resolveCollision(Object &otherObj, double dt) {
  ...
  Vec2 frictionForce = simParams.objFrictionCoeff * -contactPointVel * reactiveForce.mag();
...
void Object::move(double dt, Vec2 outsideEnv) {
  if (outsideEnv.mag() > 0.01) {
    Vec2 reactiveForce = -((_k * outsideEnv) + (_b * _vel));
